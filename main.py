...
def get_data_from_block(df, block, param):
    totals_series = pd.Series(df[block])
    transactions_values = totals_series.apply(lambda x: x.get(param))
...
def transactions_by_date(data):
    df = data.copy()
    transactions_values = get_data_from_block(df, 'totals', 'transactions')
...
def traffic_by_date(data):
    ...
    """
    df = data.copy()
    total_page_views = get_data_from_block(df, 'totals', 'pageviews')
...
def transactions_by_browser(data):
    # 10. Which browsers on specific dates had the most transactions in September 2016?¶
    df = data.copy()

    transactions_values = get_data_from_block(df, 'totals', 'transactions')
    df['total_transactions'] = transactions_values

    browser_values = get_data_from_block(df, 'device', 'browser')
...
def transactions_by_traffic_source(data):
    ...
    traffic_source_values = get_data_from_block(df, 'trafficSource', 'source')
